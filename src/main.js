...
const K = { h: 'hq-api-history', d: 'hq-display-msgs', mem: 'hq-business-memory', date: 'hq-start-date', key: 'hq-api-key' }
...
document.getElementById('save-key-btn').addEventListener('click', () => {
  ...
  const s = document.getElementById('key-ok'); s.style.display = 'block'
...
function setBrain(state) {
  ...
  dot.className = 'jsdot' + (state === 'thinking' ? ' thinking' : '')
  lbl.textContent = state === 'thinking' ? 'PROCESSING' : 'STANDBY'
}

/* MEMORY */
function loadMem() { const s = localStorage.getItem(K.mem); if (s) { try { mem = JSON.parse(s) } catch (e) {} } renderMem() }
function saveMem() { localStorage.setItem(K.mem, JSON.stringify(mem)); renderMem() }
function parseMem(text) {
  const clean = text.replace(/\[MEMORY\][\s\S]*?\[\/MEMORY\]/gi, '').replace(/<memory_update>[\s\S]*?<\/memory_update>/gi, '').trim()
...
function renderMem() {
  const el = document.getElementById('memdisplay'); if (!el) return
  const items = []
  ...
  if (!items.length) { el.innerHTML = '<div class="mem-empty">No memory yet</div>'; return }
...
function saveSession() {
  try { localStorage.setItem(K.h, JSON.stringify(apiHistory.slice(-100))); localStorage.setItem(K.d, JSON.stringify(displayLog.slice(-100))) } catch (e) {}
}
function loadSession() {
  const legacy = { h: ['hq-h', 'jv-h'], d: ['hq-d', 'jv-d'], mem: ['hq-m', 'jv-mem', 'hq-mem'], date: ['jv-date'], key: ['jv-key'] }
  Object.keys(legacy).forEach(canon => { if (localStorage.getItem(K[canon])) return; legacy[canon].forEach(old => { const v = localStorage.getItem(old); if (v) localStorage.setItem(K[canon], v) }) })
  loadMem()
  ...
  if (h && d) {
    try {
      ...
      const b = document.getElementById('restore-banner'); b.classList.add('show')
...
document.getElementById('exp').addEventListener('click', () => {
  ...
  const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([txt], { type: 'text/plain' })); a.download = 'jarvis-hq-' + new Date().toISOString().split('T')[0] + '.txt'; a.click()
})

/* QUICK BTNS */
document.querySelectorAll('.qbtn').forEach(b => b.addEventListener('click', () => { document.getElementById('txt').value = b.getAttribute('data-msg'); send() }))

/* MIC */
const micBtn = document.getElementById('mic')
const SR = window.SpeechRecognition || window.webkitSpeechRecognition
if (!SR) { micBtn.style.opacity = '.3'; micBtn.disabled = true }
else {
  micBtn.addEventListener('click', () => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
      ...
      .catch(() => addMsg('agent', 'Mic access denied. Click the lock icon and allow microphone.'))
  })
}
...
function startRec() {
  ...
  recognition.onstart = () => { isRecording = true; micBtn.classList.remove('waiting'); micBtn.classList.add('recording'); micBtn.textContent = '⏹'; document.getElementById('mstat').classList.add('on'); document.getElementById('txt').placeholder = 'Listening...' }
  recognition.onresult = e => {
    ...
    const inp = document.getElementById('txt'); inp.value = final || interim
    inp.style.height = 'auto'; inp.style.height = Math.min(inp.scrollHeight, 100) + 'px'
    if (final) stopRec(true)
  }
  recognition.onerror = e => { if (e.error === 'no-speech') return; stopRec(false); if (e.error === 'not-allowed') { micPermGranted = false; addMsg('agent', 'Mic blocked. Allow microphone in browser settings.') } }
  recognition.onend = () => { if (isRecording) { isRecording = false; micBtn.classList.remove('recording', 'waiting'); micBtn.textContent = '🎤'; document.getElementById('mstat').classList.remove('on'); document.getElementById('txt').placeholder = 'Ask Jarvis anything...' } }
...
function stopRec(andSend) {
  ...
  document.getElementById('mstat').classList.remove('on'); document.getElementById('txt').placeholder = 'Ask Jarvis anything...'
...
function showTyping() { const msgs = document.getElementById('msgs'); const r = document.createElement('div'); r.className = 't-row'; r.id = 'ti'; r.innerHTML = '<div class="mav agent">🤖</div><div class="t-bub"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div>'; msgs.appendChild(r); msgs.scrollTop = msgs.scrollHeight }
function hideTyping() { document.getElementById('ti')?.remove() }
function md(t) { return t.split('\n').map(l => { if (l.startsWith('# ')) return `<h1>${inl(l.slice(2))}</h1>`; if (l.startsWith('## ')) return `<h2>${inl(l.slice(3))}</h2>`; if (l.startsWith('### ')) return `<h3>${inl(l.slice(4))}</h3>`; if (l.startsWith('- ')) return `<p class="li">• ${inl(l.slice(2))}</p>`; if (/^\d+\.\s/.test(l)) return `<p class="li">${inl(l)}</p>`; if (l === '') return '<br>'; return `<p>${inl(l)}</p>` }).join('') }
function inl(t) { return t.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code>$1</code>') }

function welcome() { addMsg('agent', "Online. I'm **Jarvis**, your DinkKingdom intelligence system. You're floating in space — drag the galaxy behind you. Ask me anything. Say **\"what's next\"** for your next move.") }
...
async function send() {
  ...
  try {
    ...
    saveSession()
  } catch (err) { hideTyping(); setBrain('idle'); addMsg('agent', 'Connection error. Check your API key.'); console.error(err) }
  finally { isLoading = false; document.getElementById('send').disabled = false }
}
...
if (!restored) welcome()
else addMsg('agent', 'Systems online. Welcome back, Ethan.')
